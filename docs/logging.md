...
Seq's free tier caps total stored event volume. The instance is configured for **30-day retention** via a retention policy in the Seq UI (Settings → Retention). The file sink remains at 14 days; Seq is the longer-window store for the rare back-investigation. If volume ever pushes against the free-tier cap before 30 days, drop the policy to whatever fits — the file sink covers the recent window regardless.
